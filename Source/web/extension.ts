	...
	if (vscode.workspace.workspaceFolders) {
		...
		for (const folder of vscode.workspace.workspaceFolders) {
			if (kind === "any") {
				...
				if (
					(kind === "project" && pxtJSONExists) ||
					(kind === "empty" && !pxtJSONExists)
				) {
					folders.push(folder);
				}
			}
	...
	await vscode.window.withProgress(
		{
			location: vscode.ProgressLocation.Notification,
			title: isTemplate
				? vscode.l10n.t("Downloading template...")
				: vscode.l10n.t("Downloading URL..."),
			cancellable: false,
		},
		async (progress) => {
			try {
